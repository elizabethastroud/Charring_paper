data<-read.csv("~/OneDrive - Nexus365/Feedsax/Charring results/CharringdataOct29N1alldata.csv")
library(plyr)
library(nlme)
library(beeswarm)
...
par(mar=c(3,0,1,0))
batch2 <- batch[batch$Species==unique(batch$Species)[1],]
plot(batch2$pcC, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, xlim=c(-1, 50), main=taxon.names[1], ylim=c(30, 90))
axis(2)
box()
#abline(h=0)
#abline(h=72.5)
...
# data2<-data2[data2$ID!="BAR0A",]
# data2<-data2[data2$ID!="BAR0B",]
...
lmbar<-lm(normd15N28thoct~char, data=taxon.1)
lmbar<-lm(normd15N28thoct~time+temp, data=taxon.1)
...
lmBW<-lm(normd15N28thoct~time+temp, data=taxon.2)
...
lmOat<-lm(normd15N28thoct~time+temp, data=taxon.3)
...
lmRye<-lm(normd15N28thoct~time+temp, data=taxon.4)
...
#215-300 (all data)
charred.lm2 <- lm(normd15N28thoct ~ Species + temp + time, data=just.charred)
summary(charred.lm2)


no300<-just.charred[just.charred$temp!="300",]
no300.lm2 <- lm(normd15N28thoct ~ Species + temp + time, data=no300) 
summary(no300.lm2)



no215<-just.charred[just.charred$temp!="215",]
no215.lm2 <- lm(normd15N28thoct ~ Species + temp + time, data=no215) 
summary(no215.lm2)


no215300<-no215[no215$temp!="300",]
no215300.lm2 <- lm(normd15N28thoct ~ Species + temp + time, data=no215300) 
summary(no215300.lm2)
...
summary3 <- ddply(data3, c("Species", "temp", "time"), function(x) c( d15N=mean(x$normd15N28thoct), sd=sd(x2$normd15N28thoct), pcN=mean(x$pcN), CN=mean(x$CN_Crun), charoff=mean(x$Ncharoff)))
...
dev.off()
#########Seperating the wheat/barley of feedsax from the wheat/barley of agricurb - so offset is calculated from known uncharred material. 

data2<-data3

data2.0<-data2[data2$ID=="FT0A"|data2$ID=="FT0B"|data2$ID=="FT0C"|data2$ID=="BAR0B"|data2$ID=="BAR0C"|data2$ID=="BAR0A"|data2$ID=="BAR0D"|data2$ID=="BAR0E"|data2$ID=="BAR0F",]
data2.1<-data2[data2$temp=="300",]
data2.2<-data2.1[data2.1$Species=="Barley"|data2.1$Species=="BW",]
data2.3<-rbind(data2.0, data2.2)
summary <- data.frame(ddply(data2.3, c("Species", "temp", "time"), nrow))
summary2 <- ddply(data2.3, c("Species", "temp", "time"), function(x) c(d15N=mean(x$normd15N), sd=sd(x$normd15N), pcN=mean(x$pcN), d13C=mean(x$normd13C), sd=sd(x$normd13C), pcN=mean(x$pcC)))
#summary2 <- ddply(data, c("Species", "temp", "time"), function(x) c( d13C=mean(x$normd13C), sd=sd(x$normd13C), pcC=mean(x$pcC), CN=mean(x$CN_Crun)))
Ncharoff <- rep(0, nrow(data2.3))
for (i in 1:length(unique(data2.3$Species))){
offset.start <- summary2$d15N[summary2$Species==unique(data2.3$Species)[i] & summary2$time==0]
offsetted.values <- (data2.3$normd15N[data2.3$Species==unique(data2.3$Species)[i]])-offset.start
Ncharoff[data2.3$Species==unique(data2.3$Species)[i]] <- offsetted.values}

data4.1 <- data.frame(data2.3, Ncharoff)

data2.4<-data2[data2$Species=="rye"|data2$Species=="oat",]
data2.5<-data2[data2$ID!="FT0A" & data2$ID!="FT0B" & data2$ID!="FT0C"& data2$ID!="BAR0B"& data2$ID!="BAR0C"& data2$ID!="BAR0A",]
data2.6<-data2.5[data2.5$Species!="rye" & data2.5$Species!="oat",]
data2.7<-data2.6[data2.6$TT2!="Barley30024" & data2.6$TT2!="Barley3004" & data2.6$TT2!="Barley3008" & data2.6$TT2!="Free-T wheat30024" & data2.6$TT2!="Free-T wheat3004"& data2.6$TT2!="Free-T wheat3008",]


data2.8<-rbind(data2.4, data2.7)

summary <- data.frame(ddply(data2.8, c("Species", "temp", "time"), nrow))
summary2 <- ddply(data2.8, c("Species", "temp", "time"), function(x) c(d15N=mean(x$normd15N), sd=sd(x$normd15N), pcN=mean(x$pcN), d13C=mean(x$normd13C), sd=sd(x$normd13C), pcN=mean(x$pcC)))
#summary2 <- ddply(data, c("Species", "temp", "time"), function(x) c( d13C=mean(x$normd13C), sd=sd(x$normd13C), pcC=mean(x$pcC), CN=mean(x$CN_Crun)))
Ncharoff <- rep(0, nrow(data2.8))
for (i in 1:length(unique(data2.8$Species))){
offset.start <- summary2$d15N[summary2$Species==unique(data2.8$Species)[i] & summary2$time==0]
offsetted.values <- (data2.8$normd15N[data2.8$Species==unique(data2.8$Species)[i]])-offset.start
Ncharoff[data2.8$Species==unique(data2.8$Species)[i]] <- offsetted.values}

data4.2 <- data.frame(data2.8, Ncharoff)
...
par(mar=c(3,0,1,0))
batch2 <- batch[batch$Species==unique(batch$Species)[1],]
plot(batch2$Ncharoff.1, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, ylim=c(-1.5,2.7), xlim=c(-1, 50), main=taxon.names[1])
#legend("topleft", c( "Uncharred","215", "230", "245", "260",  "4h", "8h", "24h"), pch=c(8, 22,22,22,22,21,22,23), col="black", pt.bg=c(NA, "white", "lightgray", "darkgray", "black", "white", "white", "white"), bg="white", pt.cex=c(1.2,2, 2, 2, 2, 1.2,1.2,1.2), cex=1.2, box.lty=0)
axis(2)
box()
abline(h=0)
...
par(mar=c(3,0,1,0))
batch2 <- batch[batch$Species==unique(batch$Species)[i],]
plot(batch2$Ncharoff.1, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, ylim=c(-1.5, 2.7), xlim=c(-1, 50), main=taxon.names[i])
#axis(2)
box()
abline(h=0)
}
...
par(mar=c(3,0,1,0))
batch2 <- batch[batch$Species==unique(batch$Species)[1],]
plot(batch2$pcN, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, xlim=c(-1, 50), main=taxon.names[1], ylim=c(0, 6))
axis(2)
box()
mtext("% N", side=2, line=2, cex=1)
# batch2 <- batch[batch$Species==unique(batch$Species)[2],]
# plot(batch2$pcN, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, xlim=c(-1, 50), main=taxon.names[2], ylim=c(0, 6))

# box()
# batch2 <- batch[batch$Species==unique(batch$Species)[3],]
# plot(batch2$pcN, bg=c(col.list), pch=c(pch.list), ylab="", axes=F, xlab="", cex=1.5, xlim=c(-1, 50), main=taxon.names[3], ylim=c(0, 6))
...
T2<-paste(data$Species, data2$temp, sep= "")
         data2<-data.frame(data2,T2)
#####work out which Rye to use from the repeats of 245 24h
RyeR<-data2[data2$TT2=="rye24524",]
RyeR$ID<-gsub('A','',RyeR$ID)
RyeR$ID<-gsub('B','',RyeR$ID)
...
s215300<-data3[data3$temp=="215"|data3$temp=="300",]
x<-as.numeric(s215300$normd15N28thoct)
y<-as.factor(s215300$T2)

car::leveneTest(x~y)

#####Anova######
speciesmodel<-aov(x~y)
summary(speciesmodel)

library(multcomp)
posthoc<-glht(speciesmodel, linfct=mcp(y="Tukey"))
summary(posthoc)
confint(posthoc)

s215230<-data3[data3$temp=="215"|data3$temp=="230",]
s245300<-data3[data3$temp!="215"|data3$temp!="230",]


boxplot(as.numeric(s215$normd15N28thoct)~ s215$Species)
boxplot(as.numeric(s300$normd15N28thoct)~ s300$Species)

rye<-s215300[s215300$Species=="rye",]
boxplot(as.numeric(rye$normd15N28thoct)~rye$temp)

t.test(as.numeric(rye$normd15N28thoct)~ rye$temp)

t.test(rye$normd13C~ rye$temp)
oat<-s215300[s215300$Species=="oat",]
boxplot(as.numeric(oat$normd15N28thoct)~oat$temp)

t.test(as.numeric(oat$normd15N28thoct)~ oat$temp)

t.test(oat$normd13C~ oat$temp)


bar<-s215300[s215300$Species=="Barley",]
boxplot(as.numeric(bar$normd15N28thoct)~bar$temp)

t.test(as.numeric(bar$normd15N28thoct)~ bar$temp)

t.test(bar$normd13C~ bar$temp)



bw<-s215300[s215300$Species=="BW",]
boxplot(as.numeric(bw$normd15N28thoct)~bw$temp)

t.test(as.numeric(bw$normd15N28thoct)~ bw$temp)

t.test(bw$normd13C~ bw$temp)
boxplot(as.numeric(bw$normd13C)~bw$temp)


summarycharred<- ddply(rye, c("temp"), function(x) c(d15N=mean(as.numeric(x$normd15N28thoct)), sd=sd(x$normd15N28thoct), max=max(x$normd15N28thoct), min=min(x$normd15N28thoct), d13C=mean(x$normd13C), sd=sd(x$normd13C), max=max(x$normd13C), min=min(x$normd13C)))
